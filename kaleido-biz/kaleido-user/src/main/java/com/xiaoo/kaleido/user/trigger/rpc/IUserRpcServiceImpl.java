...
import com.xiaoo.kaleido.api.user.IUserRpcService;
import com.xiaoo.kaleido.api.user.constant.UserRpcConstants;
...
@DubboService(
        version = UserRpcConstants.DUBBO_VERSION,
        group = UserRpcConstants.DUBBO_GROUP,
        timeout = UserRpcConstants.DEFAULT_TIMEOUT
)
public class IUserRpcServiceImpl implements IUserRpcService {
    ...
    public Result<String> createUser(@Valid CreateUserRpcRequest request) {
        try {
            log.debug("RPC调用：创建用户，telephone={}", request.getTelephone());
            ...
            log.info("RPC调用成功：创建用户，userId={}, telephone={}", userId, request.getTelephone());
            return Result.success(userId);
        } catch (UserException e) {
            log.warn("RPC调用失败：创建用户，telephone={}, errorCode={}", request.getTelephone(), e.getErrorCode());
            return Result.error(e.getErrorCode(), e.getMessage());
        } catch (Exception e) {
            log.error("RPC调用异常：创建用户，telephone={}", request.getTelephone(), e);
            return Result.error(UserErrorCode.USER_OPERATE_FAILED.getCode(), "系统异常");
        }
    }
