package com.xiaoo.kaleido.ai.application.registry;

import com.xiaoo.kaleido.ai.application.factory.AgentChatClientFactory;
...
public class AgentRegistry {
     ...
     * value: Agent配置
     */
    private final ConcurrentHashMap<String, AgentAggregate> agentConfigCache = new ConcurrentHashMap<>();
    ...
    public void init() {
        ...
        loadEnabledAgents();
        log.info("Agent注册中心初始化完成，已加载Agent数量: {}", chatClientCache.size());
    }
    ...
    public void unregisterAgent(String agentId) {
        ChatClient removedClient = chatClientCache.remove(agentId);
        AgentAggregate removedConfig = agentConfigCache.remove(agentId);
    ...
    public ChatClient getChatClient(String agentId) {
        // 尝试从缓存获取
        ChatClient chatClient = chatClientCache.get(agentId);
        if (chatClient != null) {
            return chatClient;
        }

        // 缓存未命中，尝试加载并创建
        return chatClientCache.computeIfAbsent(agentId, id -> {
            AgentAggregate agent = loadAgent(id);
    ...
    public boolean isAgentRegistered(String agentId) {
        return chatClientCache.containsKey(agentId);
    }
    ...
    public void cleanupInvalidAgents() {
        ...
        int initialSize = chatClientCache.size();

        chatClientCache.keySet().removeIf(agentId -> {
            AgentAggregate agent = loadAgent(agentId);
            if (agent == null || agent.getStatus() != AgentStatus.NORMAL) {
                log.debug("清理无效Agent缓存，Agent ID: {}", agentId);
                agentConfigCache.remove(agentId);
                return true;
            }
            return false;
        });

        int cleanedCount = initialSize - chatClientCache.size();
        if (cleanedCount > 0) {
