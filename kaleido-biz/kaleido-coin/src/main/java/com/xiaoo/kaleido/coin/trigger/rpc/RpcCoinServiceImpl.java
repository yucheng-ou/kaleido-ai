...
public class RpcCoinServiceImpl implements IRpcCoinService {
    ...
    @Override
    public Result<String> initAccount(@NotBlank String userId, @Valid InitAccountCommand command) {
    ...
    @Override
    public Result<Void> processInviteReward(@NotBlank String userId, @Valid ProcessInviteRewardCommand command) {
    ...
    @Override
    public Result<Void> processLocationCreation(@NotBlank String userId, @Valid ProcessLocationCreationCommand command) {
    ...
    @Override
    public Result<Void> processOutfitCreation(@NotBlank String userId, @Valid ProcessOutfitCreationCommand command) {
    ...
    @Override
    public Result<Void> deposit(@NotBlank String userId, @Valid DepositCommand command) {
    ...
    @Override
    public Result<Void> withdraw(@NotBlank String userId, @Valid WithdrawCommand command) {
    ...
    @Override
    public Result<Void> processRecommendGeneration(@NotBlank String userId, @NotBlank String recommendRecordId) {
        log.info("RPC服务：处理推荐生成扣费，用户ID：{}，推荐记录ID：{}",
                userId, recommendRecordId);
        coinCommandService.processRecommendGeneration(userId, recommendRecordId);
        return Result.success();
    }

    @Override
    public Result<Boolean> checkBalance(@NotBlank String userId, @NotNull CoinBizTypeEnum bizType) {
        log.info("RPC服务：校验金币是否足够，用户ID：{}，业务类型：{}", userId, bizType);
        boolean sufficient = coinCommandService.checkBalance(userId, bizType);
